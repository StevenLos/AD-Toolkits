...
param(
    ...
    [string]$DcHealthRelationshipCsv,
...
function Test-DcHealthRelationshipCsv {
    ...
    $allowedObjectTypes = @("DomainController", "ADDomain", "ADSite", "FSMORole", "TimeSource", "FindingCategory", "ExternalDependency")
    $allowedRelationships = @("ServesDomain", "LocatedInSite", "HoldsFsmo", "UsesTimeSource", "HasFinding", "DependsOn")
    ...
    foreach ($row in $rows) {
        $rowNumber++
        foreach ($requiredValueColumn in @("DcHealthEdgeId", "Source", "SourceType", "Relationship", "Target", "TargetType")) {
        ...
        $edgeId = Get-CsvColumnValue -Row $row -ColumnName "DcHealthEdgeId"
        if ($edgeIds.ContainsKey($edgeId)) {
            throw "CSV row $rowNumber has duplicate DcHealthEdgeId '$edgeId'."
        }
        $edgeIds[$edgeId] = $true
        ...
        $severity = Get-CsvColumnValue -Row $row -ColumnName "Severity"
        if (-not (Test-AllowedValue -Value $severity -AllowedValues $allowedSeverity)) {
            throw "CSV row $rowNumber column 'Severity' has unsupported value '$severity'."
        }
    }
...
function Convert-SvgToPng {
    ...
    $userDataDir = Join-Path ([System.IO.Path]::GetTempPath()) ("dc-health-render-" + [System.Guid]::NewGuid().ToString("N"))
...
$csvPath = (Resolve-Path -LiteralPath $DcHealthRelationshipCsv).Path
$validation = Test-DcHealthRelationshipCsv -Path $csvPath

if ($RendererPath) {
    $renderer = $RendererPath
}
else {
    $renderer = Join-Path (Split-Path -Parent $PSScriptRoot) "03-render-from-discovery\Convert-ADDomainControllerHealthInventoryToSvg.py"
}
...
$inventoryJson = Join-Path $outputDirectory.FullName "$safeName.inventory.json"
$detailsCsv = Join-Path $outputDirectory.FullName "$safeName.dc-health-relationship-details.csv"
...
Invoke-ExternalCommand -FilePath $python -ArgumentList @(
    ...
    "--output", $combinedSvg,
    "--view", "combined",
    "--inventory-output", $inventoryJson,
