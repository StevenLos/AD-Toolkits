[CmdletBinding()]
param(
    [string]$CurrentCsv = ".\output\02-current-state-images\current-state.dc-health-relationship-details.csv",

    [string]$OutputCsv = ".\input\manual-csv\future-state.dc-health-relationship-details.csv",
...
function Assert-RequiredColumns {
    ...
    foreach ($requiredColumn in @("DcHealthEdgeId", "Source", "SourceType", "Relationship", "Target", "TargetType")) {
...
[pscustomobject]@{
    ...
    NextStep = "Edit FutureStateCsv, then render it with New-ADDomainControllerHealthMapImagesFromCsv.ps1 -DcHealthRelationshipCsv FutureStateCsv."
}
